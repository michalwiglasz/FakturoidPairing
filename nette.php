...
/**
 * Nette Framework (version 2.0-dev ba8b815 released on 2010-11-11)
 *
...
REVISION='ba8b815 released on 2010-11-11';public
...
run(){$httpRequest=$this->getHttpRequest();$httpResponse=$this->getHttpResponse();$session=$this->getSession();if(!$session->isStarted()&&$session->exists()){$session->start();}if($this->allowedMethods){$method=$httpRequest->getMethod();if(!in_array($method,$this->allowedMethods,TRUE)){$httpResponse->setCode(IHttpResponse::S501_NOT_IMPLEMENTED);$httpResponse->setHeader('Allow',implode(',',$this->allowedMethods));echo'<h1>Method '.htmlSpecialChars($method).' is not implemented</h1>';return;}}$request=NULL;$repeatedError=FALSE;do{try{if(count($this->requests)>self::$maxLoop){throw
new
NApplicationException('Too many loops detected in application life cycle.');}if(!$request){$this->onStartup($this);$router=$this->getRouter();NDebug::addPanel(new
...
formatLayoutTemplateFiles($presenter,$layout){$appDir=NEnvironment::getVariable('appDir');$path='/'.str_replace(':','Module/',$presenter);$pathP=substr_replace($path,'/templates',strrpos($path,'/'),0);$list=array("$appDir$pathP/@$layout.phtml","$appDir$pathP.@$layout.phtml");while(($path=substr($path,0,strrpos($path,'/')))!==FALSE){$list[]="$appDir$path/templates/@$layout.phtml";}return$list;}function
...
NPresenterRequest($presenter,NPresenterRequest::FORWARD,$args,array(),array());$this->lastCreatedRequestFlag=array('current'=>$current);if($mode==='forward')return;$uri=$router->constructUrl($this->lastCreatedRequest,$httpRequest->getUri());if($uri===NULL){unset($args[self::ACTION_KEY]);$params=urldecode(http_build_query($args,NULL,', '));throw
new
NInvalidLinkException("No route for $presenter:$action($params)");}if($mode==='link'&&$scheme===FALSE&&!$this->absoluteUrls){$hostUri=$httpRequest->getUri()->getHostUri();if(strncmp($uri,$hostUri,strlen($hostUri))===0){$uri=substr($uri,strlen($hostUri));}}return$uri.$fragment;}private
static
function
...
getConstructor(){return($ref=parent::getConstructor())?NMethodReflection::import($ref):NULL;}function
...
new
NInvalidPresenterException("Cannot load presenter '$name', class '$class' is not Nette\\Application\\IPresenter implementor.");}if($reflection->isAbstract()){throw
new
...
/><?php if(empty($this->request)):?>no route<?php else:echo$this->request->getPresenterName().':'.(isset($this->request->params[NPresenter::ACTION_KEY])?$this->request->params[NPresenter::ACTION_KEY]:NPresenter::$defaultAction);endif?>
<?php }function
renderPanel(){?>
...
<?php if(empty($this->request)):?>
	no route
<?php else:?>
	<?php echo$this->request->getPresenterName().':'.(isset($this->request->params[NPresenter::ACTION_KEY])?$this->request->params[NPresenter::ACTION_KEY]:NPresenter::$defaultAction)?>
<?php endif?>
</h1>
...
new
InvalidArgumentException("Namespace name contains forbidden NUL character.");}}function
...
offsetGet($key){$key=is_scalar($key)?(string)$key:serialize($key);if($this->key===$key){return$this->data;}$this->key=$key;$this->data=$this->storage->read($this->namespace.self::NAMESPACE_SEPARATOR.md5($key));return$this->data;}function
...
new
InvalidStateException("Cannot open journal file '$this->file'.");}if(!($this->handle=@fopen($this->file,'rb'))){$this->handle=NULL;}else{list(,$magic,$sectionCount)=unpack('N2',fread($this->handle,2*self::INT32));if($magic!==self::MAGIC){fclose($this->handle);throw
new
InvalidStateException("Malformed journal file '$this->file'.");}for($i=0;$i<$sectionCount;++$i){list(,$name,$offset,$keyLength,$keyCount)=unpack('N4',fread($this->handle,4*self::INT32));$this->sections[$name]=(object)array('offset'=>$offset,'keyLength'=>$keyLength,'keyCount'=>$keyCount);}}clearstatcache();if(($mtime=@filemtime($this->file))===FALSE){$mtime=0;}}while($this->mtime<$mtime);if(!($this->logHandle=@fopen($logfile=$this->file.self::EXTLOG,'a+b'))){throw
new
InvalidStateException("Cannot open logfile '$logfile' for journal.");}$doMergeFirst=FALSE;$openNewLog=FALSE;$reopen=FALSE;if(flock($this->logHandle,LOCK_SH|LOCK_NB)){if(file_exists($logfile=$this->file.self::EXTLOGNEW)){if(($logmtime=@filemtime($this->file.self::EXTLOG))===FALSE){throw
new
InvalidStateException("Cannot determine modification time of logfile '$this->file".self::EXTLOG."'.");}if($logmtime<$this->mtime){fclose($this->logHandle);if(!@rename($this->file.self::EXTLOGNEW,$this->file.self::EXTLOG)){clearstatcache();if(!file_exists($this->file.self::EXTLOGNEW)){$reopen=TRUE;}else{$openNewLog=TRUE;}}else{$reopen=TRUE;}}else{if(!$this->rebuild()){$doMergeFirst=TRUE;$openNewLog=TRUE;}}}}else{$doMergeFirst=TRUE;$openNewLog=TRUE;}if($reopen&&$openNewLog){throw
new
LogicException('Something bad with algorithm.');}if($doMergeFirst){$this->logMerge=$this->mergeLogFile($this->logHandle,0);}if($reopen){fclose($this->logHandle);if(!($this->logHandle=@fopen($logfile=$this->file.self::EXTLOG,'a+b'))){throw
new
InvalidStateException("Cannot open logfile '$logfile'.");}if(!flock($this->logHandle,LOCK_SH)){throw
new
InvalidStateException('Cannot acquite shared lock on log.');}}if($openNewLog){fclose($this->logHandle);if(!($this->logHandle=@fopen($logfile=$this->file.self::EXTLOGNEW,'a+b'))){throw
new
InvalidStateException("Cannot open logfile '$logfile'.");}$this->isLogNew=TRUE;}$this->logMerge=$this->mergeLogFile($this->logHandle,0,$this->logMerge);$this->logMergeP=ftell($this->logHandle);if($this->logMergeP===0){if(!flock($this->logHandle,LOCK_EX)){throw
new
InvalidStateException('Cannot acquite exclusive lock on log.');}$data=serialize(array());$data=pack('N',strlen($data)).$data;$written=fwrite($this->logHandle,$data);if($written===FALSE||$written!==strlen($data)){throw
new
InvalidStateException('Cannot write empty packet to log.');}if(!flock($this->logHandle,LOCK_SH)){throw
new
InvalidStateException('Cannot acquite shared lock on log.');}}}function
write($key,array$dependencies){$log=array();$delete=$this->get(self::ENTRIES,$key);if($delete!==NULL&&isset($delete[$key])){foreach($delete[$key]as$id){list($sectionName,$k)=explode(':',$id,2);$sectionName=intval($sectionName);if(!isset($log[$sectionName])){$log[$sectionName]=array();}if(!isset($log[$sectionName][self::DELETE])){$log[$sectionName][self::DELETE]=array();}$log[$sectionName][self::DELETE][$k][]=$key;}}if(!empty($dependencies[NCache::TAGS])){if(!isset($log[self::TAGS])){$log[self::TAGS]=array();}if(!isset($log[self::TAGS][self::ADD])){$log[self::TAGS][self::ADD]=array();}foreach((array)$dependencies[NCache::TAGS]as$tag){$log[self::TAGS][self::ADD][$tag]=(array)$key;}}if(!empty($dependencies[NCache::PRIORITY])){if(!isset($log[self::PRIORITY])){$log[self::PRIORITY]=array();}if(!isset($log[self::PRIORITY][self::ADD])){$log[self::PRIORITY][self::ADD]=array();}$log[self::PRIORITY][self::ADD][sprintf('%010u',(int)$dependencies[NCache::PRIORITY])]=(array)$key;}if(empty($log)){return;}$entriesSection=array(self::ADD=>array());foreach($log
...
new
InvalidStateException('Cannot write to log.');}if(!$this->isLogNew){fseek($this->logHandle,0,SEEK_END);$size=ftell($this->logHandle);if($size>self::LOGMAXSIZE){$this->rebuild();}}return
TRUE;}private
function
...
new
InvalidStateException("Cannot reopen logfile '$logfile'.");}$this->logMerge=array();$this->logMergeP=0;if(!($this->handle=@fopen($this->file,'rb'))){throw
new
InvalidStateException("Cannot reopen file '$this->file'.");}clearstatcache();$this->mtime=(int)@filemtime($this->file);$this->sections=$newsections;return
TRUE;}private
function
...
clean(array$conditions){if(!empty($conditions[NCache::ALL])){$this->log(array(self::CLEAN=>TRUE));return
NULL;}else{$log=array();$entries=array();if(!empty($conditions[NCache::TAGS])){$tagEntries=array();foreach((array)$conditions[NCache::TAGS]as$tag){$tagEntries=array_merge($tagEntries,$tagEntry=$this->get(self::TAGS,$tag));if(isset($tagEntry[$tag])){foreach($tagEntry[$tag]as$entry){$entries[]=$entry;}}}if(!empty($tagEntries)){if(!isset($log[self::TAGS])){$log[self::TAGS]=array();}$log[self::TAGS][self::DELETE]=$tagEntries;}}if(isset($conditions[NCache::PRIORITY])){$priorityEntries=$this->getLte(self::PRIORITY,sprintf('%010u',(int)$conditions[NCache::PRIORITY]));foreach($priorityEntries
...
__construct($dir,NContext$context=NULL){if(self::$useDirectories===NULL){$uniq=uniqid('_',TRUE);umask(0000);if(!@mkdir("$dir/$uniq",0777)){throw
new
...
NULL;}}private
function
verify($meta){do{if(!empty($meta[self::META_DELTA])){if(filemtime($meta[self::FILE])+$meta[self::META_DELTA]<time())break;touch($meta[self::FILE]);}elseif(!empty($meta[self::META_EXPIRE])&&$meta[self::META_EXPIRE]<time()){break;}if(!empty($meta[self::META_CALLBACKS])&&!NCache::checkCallbacks($meta[self::META_CALLBACKS])){break;}if(!empty($meta[self::META_ITEMS])){foreach($meta[self::META_ITEMS]as$depFile=>$time){$m=$this->readMeta($depFile,LOCK_SH);if($m[self::META_TIME]!==$time)break
2;if($m&&!$this->verify($m))break
2;}}return
TRUE;}while(FALSE);$this->delete($meta[self::FILE],$meta[self::HANDLE]);return
FALSE;}function
write($key,$data,array$dp){$meta=array(self::META_TIME=>microtime());if(isset($dp[NCache::EXPIRATION])){if(empty($dp[NCache::SLIDING])){$meta[self::META_EXPIRE]=$dp[NCache::EXPIRATION]+time();}else{$meta[self::META_DELTA]=(int)$dp[NCache::EXPIRATION];}}if(isset($dp[NCache::ITEMS])){foreach((array)$dp[NCache::ITEMS]as$item){$depFile=$this->getCacheFile($item);$m=$this->readMeta($depFile,LOCK_SH);$meta[self::META_ITEMS][$depFile]=$m[self::META_TIME];unset($m);}}if(isset($dp[NCache::CALLBACKS])){$meta[self::META_CALLBACKS]=$dp[NCache::CALLBACKS];}$cacheFile=$this->getCacheFile($key);if($this->useDirs&&!is_dir($dir=dirname($cacheFile))){umask(0000);if(!mkdir($dir,0777)){return;}}$handle=@fopen($cacheFile,'r+b');if(!$handle){$handle=fopen($cacheFile,'wb');if(!$handle){return;}}if(isset($dp[NCache::TAGS])||isset($dp[NCache::PRIORITY])){if(!$this->context){throw
new
InvalidStateException('CacheJournal has not been provided.');}$this->getJournal()->write($cacheFile,$dp);}flock($handle,LOCK_EX);ftruncate($handle,0);if(!is_string($data)){$data=serialize($data);$meta[self::META_SERIALIZED]=TRUE;}$head=serialize($meta).'?>';$head='<?php //netteCache[01]'.str_pad((string)strlen($head),6,'0',STR_PAD_LEFT).$head;$headLen=strlen($head);$dataLen=strlen($data);do{if(fwrite($handle,str_repeat("\x00",$headLen),$headLen)!==$headLen){break;}if(fwrite($handle,$data,$dataLen)!==$dataLen){break;}fseek($handle,0);if(fwrite($handle,$head,$headLen)!==$headLen){break;}fclose($handle);return
...
function
getJournal(){return$this->context->getService('Nette\\Caching\\ICacheJournal');}}class
NMemcachedStorage
extends
NObject
implements
...
__construct($host='localhost',$port=11211,$prefix='',NContext$context=NULL){if(!self::isAvailable()){throw
new
NotSupportedException("PHP extension 'memcache' is not loaded.");}$this->prefix=$prefix;$this->context=$context;$this->memcache=new
...
clean(array$conds){if(!empty($conds[NCache::ALL])){$this->memcache->flush();}elseif($this->context){foreach($this->getJournal()->clean($conds)as$entry){$this->memcache->delete($entry,0);}}}protected
function
getJournal(){return$this->context->getService('Nette\\Caching\\ICacheJournal');}}class
NMemoryStorage
extends
...
new
NotSupportedException("SQLite or SQLite3 extension is required for storing tags and priorities.");}$this->database=extension_loaded('sqlite')?new
SQLiteMimic($file):new
SQLite3($file);@$this->database->exec('CREATE TABLE cache (entry VARCHAR NOT NULL, priority INTEGER, tag VARCHAR); '.'CREATE INDEX IDX_ENTRY ON cache (entry); '.'CREATE INDEX IDX_PRI ON cache (priority); '.'CREATE INDEX IDX_TAG ON cache (tag);');}function
write($key,array$dependencies){$entry=$this->database->escapeString($key);$query='';if(!empty($dependencies[NCache::TAGS])){foreach((array)$dependencies[NCache::TAGS]as$tag){$query.="INSERT INTO cache (entry, tag) VALUES ('$entry', '".$this->database->escapeString($tag)."'); ";}}if(!empty($dependencies[NCache::PRIORITY])){$query.="INSERT INTO cache (entry, priority) VALUES ('$entry', '".((int)$dependencies[NCache::PRIORITY])."'); ";}if(!$this->database->exec("BEGIN; DELETE FROM cache WHERE entry = '$entry'; $query COMMIT;")){$this->database->exec('ROLLBACK');return
FALSE;}return
TRUE;}function
clean(array$conditions){if(!empty($conditions[NCache::ALL])){$this->database->exec('DELETE FROM CACHE;');return;}$query=array();if(!empty($conditions[NCache::TAGS])){$tags=array();foreach((array)$conditions[NCache::TAGS]as$tag){$tags[]="'".$this->database->escapeString($tag)."'";}$query[]='tag IN('.implode(', ',$tags).')';}if(isset($conditions[NCache::PRIORITY])){$query[]='priority <= '.((int)$conditions[NCache::PRIORITY]);}$entries=array();if(!empty($query)){$query=implode(' OR ',$query);$result=$this->database->query("SELECT entry FROM cache WHERE $query");if($result
instanceof
SQLiteResult){while($entry=$result->fetchSingle())$entries[]=$entry;}else{while($entry=$result->fetchArray(SQLITE3_NUM))$entries[]=$entry[0];}$this->database->exec("DELETE FROM cache WHERE $query");}return$entries;}}if(class_exists('SQLiteDatabase')){class
SQLiteMimic
extends
SQLiteDatabase{function
exec($sql){return$this->queryExec($sql);}function
escapeString($s){return
sqlite_escape_string($s);}}}class
NConfig
implements
...
"*");if(b[1]){for(var d=[],f=RegExp("(^|\\s)"+b[1]+"(\\s|$)"),i=0,k=c.length;i<k;i++)f.test(c[i].className)&&d.push(c[i]);return d}else return c}},dom:function(){return this[0]},each:function(a,b){for(var c=0,d;c<this.length;c++)if((d=a.apply(this[c],b||[]))!==void 0)return d;return this}});var h=Nette.Q.factory,e=Nette.Q.implement;e({bind:function(a,b){if(document.addEventListener&&(a==="mouseenter"||a==="mouseleave")){var c=b;a=a==="mouseenter"?"mouseover":"mouseout";b=function(g){for(var j=g.relatedTarget;j;j=
...
function
createApplication(){if(NEnvironment::getVariable('baseUri',NULL)===NULL){NEnvironment::setVariable('baseUri',NEnvironment::getHttpRequest()->getUri()->getBaseUri());}$context=clone
NEnvironment::getContext();$context->addService('Nette\\Application\\IRouter','NMultiRouter');if(!$context->hasService('Nette\\Application\\IPresenterLoader')){$context->addService('Nette\\Application\\IPresenterLoader',callback(create_function('','
			...
			')));}$application=new
NApplication;$application->setContext($context);$application->catchExceptions=NEnvironment::isProduction();return$application;}static
function
...
NContext;$context->addService('Nette\\Caching\\ICacheJournal',NEnvironment::getContext());$dir=NEnvironment::getVariable('tempDir').'/cache';umask(0000);@mkdir($dir,0777);return
new
NFileStorage($dir,$context);}static
function
createCacheJournal(){{return
new
NFileJournal(NEnvironment::getVariable('tempDir').'/cache');}}static
function
createMailer($options){if(isset($options['smtp'])){return
new
...
as$rule){if(!is_string($rule->operation)){continue;}elseif($rule->type===NRule::VALIDATOR){$item=array('op'=>($rule->isNegative?'~':'').$rule->operation,'msg'=>$rules->formatMessage($rule,FALSE));}elseif($rule->type===NRule::CONDITION){$item=array('op'=>($rule->isNegative?'~':'').$rule->operation,'rules'=>self::exportRules($rule->subRules),'control'=>$rule->control->getHtmlName());if($rule->subRules->getToggles()){$item['toggle']=$rule->subRules->getToggles();}}if(is_array($rule->arg)){foreach($rule->arg
...
static$defaultMessages=array(NForm::PROTECTION=>'Security token did not match. Possible CSRF attack.');private$rules=array();private$parent;private$toggles=array();private$control;function
...
function
addClass($class,$file,$time){$lClass=strtolower($class);if(isset($this->list[$lClass][0])&&$this->list[$lClass][0]!==$file&&is_file($this->list[$lClass][0])){$e=new
InvalidStateException("Ambiguous class '$class' resolution; defined in $file and in ".$this->list[$lClass][0].".");if(PHP_VERSION_ID<50300){NDebug::_exceptionHandler($e);exit;}else{throw$e;}}$this->list[$lClass]=array($file,$time,$class);}private
function
...
function
scanScript($file){$T_NAMESPACE=PHP_VERSION_ID<50300?-1:T_NAMESPACE;$T_NS_SEPARATOR=PHP_VERSION_ID<50300?-1:T_NS_SEPARATOR;$expected=FALSE;$namespace='';$level=$minLevel=0;$time=filemtime($file);$s=file_get_contents($file);if($matches=NString::match($s,'#//nette'.'loader=(\S*)#')){foreach(explode(',',$matches[1])as$name){$this->addClass($name,$file,$time);}return;}foreach(token_get_all($s)as$token){if(is_array($token)){switch($token[0]){case
T_COMMENT:case
T_DOC_COMMENT:case
...
NMailMimePart;if($content===NULL){if(!is_file($file)){throw
new
...
instanceof
NTemplateCacheStorage){$storage->hint=str_replace(dirname(dirname($this->file)),'',$this->file);}$cached=$content=$cache[$this->file];if($content===NULL){try{$content=$this->compile(file_get_contents($this->file));$content="<?php\n\n// source file: $this->file\n\n?>$content";}catch(NTemplateException$e){$e->setSourceFile($this->file);throw$e;}$cache->save($this->file,$content,array(NCache::FILES=>$this->file,NCache::EXPIRATION=>self::$cacheExpire,NCache::CONSTS=>'NFramework::REVISION'));$cache->release();$cached=$cache[$this->file];}if($cached!==NULL&&$storage
instanceof
NTemplateCacheStorage){NLimitedScope::load($cached['file'],$this->getParams());fclose($cached['handle']);}else{NLimitedScope::evaluate($content,$this->getParams());}}static
function
setCacheStorage(ICacheStorage$storage){self::$cacheStorage=$storage;}static
function
getCacheStorage(){if(self::$cacheStorage===NULL){$dir=NEnvironment::getVariable('tempDir').'/cache';umask(0000);@mkdir($dir,0777);self::$cacheStorage=new
NTemplateCacheStorage($dir);}return
self::$cacheStorage;}}class
NCachingHelper
extends
NObject{private$frame;private$key;static
function
create($key,&$parents,$args=NULL){if($args){$key.=array_intersect_key($args,range(0,count($args)));if(array_key_exists('if',$args)&&!$args['if']){return$parents[]=new
self;}}if($parents){end($parents)->frame[NCache::ITEMS][]=$key;}$cache=self::getCache();if(isset($cache[$key])){echo$cache[$key];return
FALSE;}else{$obj=new
...
initialize($filter,&$s){$this->filter=$filter;$this->nodes=array();$this->blocks=array();$this->namedBlocks=array();$this->extends=NULL;$this->uniq=substr(md5(uniqid('',TRUE)),0,10);$this->cacheCounter=0;$filter->context=NLatteFilter::CONTEXT_TEXT;$filter->escape='NTemplateHelpers::escapeHtml';}function
...
new
NLatteException("Multiple {extends} declarations are not allowed.",0,$this->filter->line);}$this->extends=$destination!=='none';return$this->extends?'$_l->extends = '.($destination==='auto'?'$layout':$this->formatString($destination)):'';}function
...
macroForeach($content){return'$iterator = $_l->its[] = new NSmartCachingIterator('.preg_replace('# +as +#i',') as ',$this->formatMacroArgs($content),1);}function
...
fetchToken(&$s){if($matches=NString::match($s,'#^((?>'.NLatteFilter::RE_STRING.'|[^\'"\s,]+)+)\s*,?\s*(.*)$#')){$s=$matches[2];return$matches[1];}return
NULL;}function
...
NString::replace($s,'#(src|href|action)\s*=\s*(["\'])(?![a-z]+:|[\x01/\\#])#','$1=$2<?php echo \\$baseUri ?>');}static
function
netteLinks($s){return
...
function
grep(array$arr,$pattern,$flags=0){NDebug::tryError();$res=preg_grep($pattern,$arr,$flags);NString::catchPregError($pattern);return$res;}}class
NContext
extends
NFreezableObject
implements
...
new
NImageMagick($file,$format);}switch($format=$info[2]){case
self::JPEG:return
new
...
new
NImageMagick($file,$format);}throw
new
...
new
NNeonException(str_replace('%s',$token,$message)."' on line ".($line-1).", column $col.");}}class
NNeonException
extends
Exception{}class
NPaginator
extends
NObject{private$base=1;private$itemsPerPage=1;private$page;private$itemCount=0;function
...
new
InvalidStateException('Critical section has already been entered.');}$handle=($tmp=fopen(NETTE_DIR.'/lockfile','r'))?$tmp:fopen(NETTE_DIR.'/lockfile','w');if(!$handle){throw
new
InvalidStateException("Unable initialize critical section (missing file '".NETTE_DIR."/lockfile').");}flock(self::$criticalSections=$handle,LOCK_EX);}static
function
leaveCriticalSection(){if(!self::$criticalSections){throw
new
InvalidStateException('Critical section has not been initialized.');}fclose(self::$criticalSections);self::$criticalSections=NULL;}}class
NHtml
extends
NObject
implements
...
new
InvalidStateException($e->getMessage());}self::$started=TRUE;if($this->regenerationNeeded){session_regenerate_id(TRUE);$this->regenerationNeeded=FALSE;}unset($_SESSION['__NT'],$_SESSION['__NS'],$_SESSION['__NM']);$nf=&$_SESSION['__NF'];if(empty($nf)){$nf=array('C'=>0);}else{$nf['C']++;}$browserKey=$this->getHttpRequest()->getCookie('nette-browser');if(!$browserKey){$browserKey=(string)lcg_value();}$browserClosed=!isset($nf['B'])||$nf['B']!==$browserKey;$nf['B']=$browserKey;$this->sendCookie();if(isset($nf['META'])){$now=time();foreach($nf['META']as$namespace=>$metadata){if(is_array($metadata)){foreach($metadata
...
setExpiration($time){if(empty($time)){return$this->setOptions(array('gc_maxlifetime'=>self::DEFAULT_FILE_LIFETIME,'cookie_lifetime'=>0));}else{$time=NTools::createDateTime($time)->format('U');return$this->setOptions(array('gc_maxlifetime'=>$time,'cookie_lifetime'=>$time));}}function
